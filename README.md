...
The header x-payload-digest contains a signature to ensure webhooks are generated by Whitebird. To verify the signature, follow these steps:
Compute an HMAC with the SHA1 hash function. 
Use the webhookSigningHash from the merchant's account, as the key and use the webhook body string as the message.
Compare the signature in the header to the computed signature

### Enable webhooks (merchant cabinet)

Open the merchant account and go to **Settings** (`{your-client-app-domain}/merchant/settings`).
In the **Properties** section, enter your public **`https://`** endpoint in **Webhook URL**, and click **Save** — Whitebird will deliver event notifications to that URL via HTTP `POST` when events occur (for example `order.completed`, `client.verified`).
